...
function Notice({ children }: { children: React.ReactNode }) {
  return (
    <Card
      padding="sm"
      className="mt-8 max-w-xl bg-surface/50 text-small leading-relaxed text-foreground"
    >
      {children}
    </Card>
  );
    ...
    <Page>
      <Section size="lg">
        ...
        {!configured ? (
          ...
          <>
            <p className="mt-8 max-w-xl text-body leading-relaxed text-foreground">
    ...
    <>
      ...
      <form action={logout} className="mt-14 border-t border-border pt-8">
        <Button
          type="submit"
          variant="link"
          className="font-mono text-label uppercase tracking-[0.18em] text-foreground hover:text-accent"
        >
          Se déconnecter
        </Button>
      </form>
